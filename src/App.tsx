...
import { Layout, Card, Button, DatePicker, Checkbox, List, Row, Col, Typography, Space, message, ConfigProvider, Divider } from 'antd';
...
function App() {
  ...
  const renderDietPlan = () => {
    if (!startDate) {
      return <Text type="secondary" style={{ textAlign: 'center', display: 'block', marginTop: 24 }}>Please select a start date for your diet plan.</Text>;
    }
    ...
    if (week < 1 || week > dietPlan.length) {
      return <Text type="danger" style={{ textAlign: 'center', display: 'block', marginTop: 24 }}>No diet plan available for this date.</Text>;
    }
    ...
    if (!dailyPlan) {
      return <Text type="danger" style={{ textAlign: 'center', display: 'block', marginTop: 24 }}>No diet plan available for this day.</Text>;
    }
    ...
    <ConfigProvider theme={theme}>
      <Layout style={{ minHeight: '100vh', backgroundColor: '#f0f2f5' }}>
        <Header style={{ backgroundColor: '#fff', borderBottom: '1px solid #f0f0f0', padding: '0 24px' }}>
          <Title level={2} style={{ color: theme.token.colorPrimary, margin: '16px 0', textAlign: 'center' }}>Switch On Diet</Title>
        </Header>
            ...
            <Row justify="space-between" align="middle">
              ...
              <Col>
                <Space direction="vertical" align="center">
                  <Title level={4} style={{ margin: 0 }}>{now.format('HH:mm:ss')}</Title>
